...
class EnergyTrackerSeeder extends Seeder
{
    ...
    {
        return 'Legt Demo-Messgeräte und historische Zählerstände an';
    }
    ...
    private static function createMeasurementDevice(User $user, string $name, string $countingType, string $countingUnit, string $description): void
    {
        ...
        $startValue = (float)$measurementDevice->meter_reading_value;
        ...
        for ($i = 1; $i <= self::SUB_MONTHS; $i++) {
            ...
            ReadingEntry::query()
                ->firstOrCreate(
                    [
                        ReadingEntry::measurement_device_id => $measurementDevice->getKey(),
